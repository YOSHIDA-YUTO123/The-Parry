//===================================================
//
// 竜巻パーティクル [PaticleTornado.h]
// Author:YUTO YOSHIDA
//
//===================================================

//***************************************************
// インクルードファイル
//***************************************************
#include "ParticleTornado.h"
#include"manager.h"
#include"slow.h"

using namespace Const; // 名前空間Constの使用

//===================================================
// コンストラクタ
//===================================================
CParticleTornado::CParticleTornado()
{
}

//===================================================
// デストラクタ
//===================================================
CParticleTornado::~CParticleTornado()
{
}

//===================================================
// 初期化処理
//===================================================
HRESULT CParticleTornado::Init(void)
{
	// 初期化処理
	if (FAILED(CParticle3D::Init()))
	{
		return E_FAIL;
	}

	return S_OK;
}

//===================================================
// 終了処理
//===================================================
void CParticleTornado::Uninit(void)
{
	// 終了処理
	CParticle3D::Uninit();
}

//===================================================
// 更新処理
//===================================================
void CParticleTornado::Update(void)
{
	// 情報の取得
	Info info = GetInfo();

	for (int nCnt = 0; nCnt < info.nNumParticle; nCnt++)
	{
		// 位置の取得
		D3DXVECTOR3 pos = info.pos;

		//// 色の取得
		//D3DXCOLOR col = info.col;

		//int nAngleMax = (info.nAngle * 2) + 1;

		//// 角度の選出
		//float fAngleX = (float)(rand() % nAngleMax - info.nAngle) * 0.01f;
		//float fAngleY = (float)(rand() % nAngleMax - info.nAngle) * 0.01f;

		//// 速さをint型に変換
		//int speed = (int)info.fSpeed;

		//// 移動量
		//float fMove = (float)(rand() % speed + (speed * 0.5f));

		//D3DXVECTOR3 moveWk = VEC3_NULL;

		//// 球状に放出する
		//moveWk.x = sinf(fAngleX) * sinf(fAngleY) * fMove;
		//moveWk.y = cosf(fAngleX) * fMove;
		//moveWk.z = sinf(fAngleX) * cosf(fAngleY) * fMove;

		//// 半径をint型に変換
		//int radius = (int)info.fRadius;

		//// 半径を選出
		//float fRadius = (float)(rand() % radius + (radius * 0.5f));

		//// 寿命の選出
		//int nLife = rand() % info.nLife - (int)(info.nLife * 0.5f);

		//// スローモーションのレベルを掛ける
		//nLife *= (int)CManager::GetSlow()->GetLevel(true);

		//if (info.nTime > 0)
		//{
		//	// エフェクトの生成
		//	auto pEffect = CEffect3D::Create(pos, fRadius, col);

		//	// エフェクトの設定処理
		//	pEffect->Set(nLife, moveWk);
		//}
	}

	// 継続時間,寿命の減少
	info.nTime--;

	// 0になったら消す
	if (info.nTime <= 0)
	{
		Uninit();
		return;
	}
	else
	{
		// 情報の設定
		SetInfo(info);
	}
}

//===================================================
// 描画処理
//===================================================
void CParticleTornado::Draw(void)
{
	// 描画処理
	CParticle3D::Draw();
}
